...
class handDetector():
    ...
    def thumbs_up(self,img):
        lmList = self.findPosition(img, draw=False)
        if not lmList:
         return None

        fingers = self.fingers_up(img)
    ...
    def  normalised_distance(self,p1,p2,img):
        ref= self.findDistance(0,9,img, draw = False)
        if ref==0:
            return 0
        return self.findDistance(p1,p2,img,draw=False)/ref
